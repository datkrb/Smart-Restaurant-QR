...
exports.createTable = async (req, res) => {
  try {
    ...
    const newTable = new Table({ table_number, capacity, location, description });
    await newTable.save();
...
exports.generateQR = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { tableId: table._id, tableNumber: table.table_number },
...
exports.downloadQRPDF = async (req, res) => {
  try {
    ...
    if (!table || !table.qr_token) {
      return res.status(404).json({ message: 'Table or QR Token not found. Please generate QR first.' });
    }
    ...
    // 2. Tiêu đề
    doc.font('Helvetica-Bold').fontSize(20).text('SCAN TO ORDER', { align: 'center' });
    doc.moveDown(0.5);
    ...
    doc.fontSize(10).font('Helvetica').text('Use your camera to scan menu', { align: 'center' });
